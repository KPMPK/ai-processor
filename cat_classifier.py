...
class PatternRedactorParameters(Parameters):
    ...
    rules: List[str] = Field(
        #default_factory=lambda: [(r'(นาย|นาง|นางสาว|นส\.)(?:\s*)(\S+)\s+(\S+)')],
        default_factory=lambda: [
            r'(นาย|นาง|นางสาว|นส\.)(?:\s*)(\S+)\s+(\S+)',  # รูปแบบเก่า (จับชื่อ-นามสกุล)
            r'^\d{13}$',  #  pattern thai national id
            r'\b(\d{4})\s(\d{4})\s(\d{4})\s(\d{4})\b'  # เพิ่ม pattern ใหม่สำหรับการจับหมายเลขบัตรเครดิต
        ],
